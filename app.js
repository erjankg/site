...
(function() {
    ...
    // ── LENIS: buttery smooth scroll (3KB, used by every awwwards site) ──
    if (window.Lenis && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        var lenis = new window.Lenis({
            ...
            smoothWheel: true,
            smoothTouch: false, // НЕ ломать нативный мобильный скролл
            wheelMultiplier: 1,
            touchMultiplier: 2,
        });
        window._lenis = lenis;

        // Sync with GSAP ticker (idiomatic Lenis+GSAP pattern)
        ...
        // Stop Lenis when modal is open (модалки имеют свой scroll)
    ...
    // ── MAGNETIC BUTTONS: курсор притягивает primary CTA (subtle, дорого) ──
    ...
    // Apply to primary buttons (not all — только важные)
    if (!('ontouchstart' in window)) {
        // Только на десктопе
        setTimeout(function(){
            initMagnetic('.btn-support, .lang-btn, .auth-btn, .header-view-btn', 0.18);
        }, 500);
    }

    // ── CURSOR BLOB: мягкий blob следует за курсором (luxury feel) ──
    if (!('ontouchstart' in window) && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        var blob = document.createElement('div');
        blob.id = 'cursorBlob';
        document.body.appendChild(blob);

        var mouseX = 0, mouseY = 0, blobX = 0, blobY = 0;
        var raf = false;
        document.addEventListener('mousemove', function(e){
            mouseX = e.clientX; mouseY = e.clientY;
            if (!raf) { raf = true; requestAnimationFrame(updateBlob); }
        });
        function updateBlob() {
            // lerp 0.15 — мягкое следование
            blobX += (mouseX - blobX) * 0.15;
            blobY += (mouseY - blobY) * 0.15;
            blob.style.transform = 'translate3d(' + (blobX - 150) + 'px,' + (blobY - 150) + 'px,0)';
            if (Math.abs(mouseX - blobX) > 0.5 || Math.abs(mouseY - blobY) > 0.5) {
                requestAnimationFrame(updateBlob);
            } else {
                raf = false;
            }
        }
        // Hover state — blob увеличивается на интерактивных элементах
        document.addEventListener('mouseover', function(e){
            if (e.target.closest('button, a, .item-card, .rune-card, .champ-cell, [onclick]')) {
    ...
    // ── LIVE SEARCH DEBOUNCE: champion search во всех инпутах ──
    ...
    // Find all search inputs and rebind their handlers
    document.querySelectorAll('input[type="search"], input.m-search, #mSearch, .search-input').forEach(function(input){
        if (input._debounceBound) return;
        input._debounceBound = true;
        var origHandler = input.oninput;
        if (origHandler) {
            var debounced = debounce(origHandler.bind(input), 120);
            input.oninput = function(e) {
                // optimistic: показываем loading state мгновенно
                input.classList.add('searching');
                debounced(e);
                clearTimeout(input._loadingTimer);
                input._loadingTimer = setTimeout(function(){
    ...
    // ── LONG-PRESS context menu (mobile, iOS-style) ──
    var _longPressTimer = null;
    var _longPressEl = null;
    function startLongPress(e) {
        var target = e.target.closest('.champ-cell, .item-card, .rune-card, .tier-chip');
        if (!target) return;
        _longPressEl = target;
        _longPressTimer = setTimeout(function(){
            if (_longPressEl) {
                if (window.navigator && window.navigator.vibrate) window.navigator.vibrate(15);
                _longPressEl.dispatchEvent(new CustomEvent('longpress', { bubbles: true, detail: { target: _longPressEl } }));
                _longPressEl.classList.add('long-pressed');
            }
        }, 500);
    }
    function cancelLongPress() {
        ...
        if (_longPressEl) _longPressEl.classList.remove('long-pressed');
        _longPressEl = null;
    }
    ...
    document.addEventListener('contextmenu', function(e){
        // Prevent default browser context menu on long-pressed targets
        if (e.target.closest('.long-pressed')) e.preventDefault();
    });
