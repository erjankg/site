...
/* минорные руны WR (3 слота × 3) — реальные имена+иконки, эффект = статы (демо-числа) */
const SHARDS=[
  {row:0,key:'brutal',      img:'brutal',       name:'Жестокость',        note:'+9 AD, +5 пробивания брони', stat:{ad:9,arPenFlat:5}},
  {row:0,key:'giant-slayer',img:'giant-slayer', name:'Убийца гигантов',   note:'+урон по живучим целям',      stat:{ad:6}},
  {row:0,key:'coup-de-grace',img:'coup-de-grace',name:'Удар милосердия',   note:'+урон по раненым целям',      stat:{ad:5}},
  {row:1,key:'axiom-arcanist',img:'axiom-arcanist',name:'Аксиома чародея', note:'+14 силы умений',             stat:{ap:14}},
  {row:1,key:'sudden-impact',img:'sudden-impact',name:'Внезапный удар',    note:'+7 пробивания брони и МС',    stat:{arPenFlat:7,mrPenFlat:7}},
  {row:1,key:'cheap-shot',  img:'cheap-shot',   name:'Дешёвый выстрел',    note:'чистый урон по контролю',     stat:{ad:5}},
  {row:2,key:'bone-plating',img:'bone-plating', name:'Костяная пластина',  note:'+55 HP, +5 брони',            stat:{hp:55,armor:5}},
  {row:2,key:'second-wind', img:'second-wind',  name:'Второе дыхание',     note:'реген после урона (+45 HP)',  stat:{hp:45}},
  {row:2,key:'last-stand',  img:'last-stand',   name:'Последний шанс',     note:'+урон на низком HP',          stat:{ad:7}},
];
...
const ITEMS = {
  ...
  sunfire:{ name:'🔥 Эгида Солнечного огня', img:'sunfire-aegis', desc:'Маг. урон/с рядом с врагами', needsRange:false, tick:true,
    fields:[ {id:'ic_myBHp2',label:'Мой бонусный HP',ph:1000,side:'my'},
             ...
             {id:'ic_eHpSun',label:'Макс HP цели',ph:2500,side:'enemy'} ],
    ...
    perTick:v=>{ const bHp=+v('ic_myBHp2')||1000,st=Math.min(4,+v('ic_stacks')||4),lv=+v('ic_eLvl')||10,mr=+v('ic_eMR2')||40;
      const base=Math.round(16+(30-16)*(lv-1)/14), raw=(base+bHp*0.01)*(1+st*0.11);
      return { dmg:raw*100/(100+mr), startHp:+v('ic_eHpSun')||2500 }; } },
  liandry:{ name:'🔮 Мучения Лиандри', img:'liandrys-torment', desc:'% макс. HP/с маг. ожог', needsRange:false, liandry:true, tick:true,
    ...
    perTick:v=>{ const hp=+v('ic_eHpMaxL')||3000,mr=+v('ic_eMR3')||40, pct=state.liandryPct||0.5;
      return { dmg:hp*(pct/100)*100/(100+mr), startHp:hp }; } },
...
function runeSlot(side){
  const cur = side==='my'?state.myKeystone:state.myKeystone;
...
/* ── ПЕР-ВКЛАДОЧНЫЕ настройки лаба (у каждой вкладки свои: размер/дизайн/кнопки) ──
   Универсальные для всех вкладок: size + design. Плюс у некоторых — свои доп. */
const TAB_SIZE   = [{id:'compact',name:'Компактный'},{id:'norm',name:'Обычный'},{id:'large',name:'Крупный'}];
...
const TAB_EXTRAS = {
  ...
  auto:  [],
  arena: ['btn'],                          // чипы режимов сверху / снизу
};
...
const DEFAULTS = {
  ...
  // ПЕР-ВКЛАДОЧНЫЕ настройки лаба: у каждой вкладки свой размер/дизайн/кнопки
  tabCfg:{
    dmg:  {size:'norm',design:'cards'},
    items:{size:'norm',design:'cards'},
    build:{size:'norm',design:'cards',btn:'bottom'},
    cmp:  {size:'norm',design:'cards',orient:'side'},
    auto: {size:'norm',design:'cards'},
    arena:{size:'norm',design:'cards',btn:'top'},
  },
  curItem:'botrk',
  itemRange:{}, liandryPct:0.5, pickIco:56,
  arena:'duel',              // режим мини-игры
};
let state = JSON.parse(JSON.stringify(DEFAULTS));
let usersetOpen = false;
let gearPick = null;   // открытый пикер снаряги: 'my:items' | 'tgt:boots' | null
let optimOpen = false; // открыт ли оптимизатор «что купить против цели»
let _fightHp = null;   // накопительный HP цели между «Разыграть» (null = с полного)
let _fightKey = '';    // ключ цели — сменился → бой сбрасывается
let LS = null;

function tcfg(tab){ tab=tab||state.tab; if(!state.tabCfg) state.tabCfg={};
  if(!state.tabCfg[tab]) state.tabCfg[tab]={size:'norm',design:'cards',btn:'top',orient:'side'};
  return state.tabCfg[tab]; }
...
function compute(){
  ...
  const ab = attackerBonus();                              // осколки рун + стаки чемпа (единый билд)
  const baseAD = +state.myBaseAD||0;
  const totalAD = baseAD + (+state.myBonusAD||0) + myG.ad + ksStat.ad + ab.ad;
  const ap = (((+state.myAP||0) + myG.ap) * (1 + myG.apAmp)) + ksStat.ap + ab.ap;
  const crit = Math.max(0, Math.min(100, (+state.myCrit||0) + myG.crit + ab.crit));
...
function resultPanel(d){
  ...
  const optim = `<div class="dc-optim-bar"><button class="dc-optbtn ${optimOpen?'on':''}" data-optim>💰 Что купить против цели${optimOpen?' ▲':' ▼'}</button></div>${optimOpen?optimPanelHTML():''}`;
  return `<div class="dc-result glassy">
    ${actions}
    <div class="dc-fxlayer" id="dcFx"></div>
    ${topRow}${eff}${optim}${formula}</div>`;
}
...
function playCombo(){
  ...
  function step(){
    if(i>=seq.length){ _playing=false; _fightHp=Math.max(0,remain);   // накопить остаток для след. комбо
      if(remain<=0) spawnFloat(fx,{text:'УБИТ ☠',big:true,xPct:50,top:18});
      setTimeout(()=>{ if(!_playing) recompute(); }, 650);   // обновить чипы «осталось / ударов до смерти» после боя
      return; }
...
function bindActions(){
  ...
  // оптимизатор «что купить против цели»
  const ob=inner.querySelector('[data-optim]'); if(ob) ob.onclick=()=>{ optimOpen=!optimOpen; renderStage(); };
  inner.querySelectorAll('[data-optadd]').forEach(b=>b.addEventListener('click',()=>{
    const [key,rep]=b.dataset.optadd.split(':');
    if(rep){ const i=state.myItems.indexOf(rep); if(i>=0)state.myItems[i]=key; }
    else if(state.myItems.length<6){ state.myItems.push(key); }
    renderStage();
  }));
}
...
function shardStats(shards){
  shards = shards || state.myShards || {};
  const a={ad:0,ap:0,hp:0,armor:0,mr:0,crit:0,as:0,ah:0,ms:0,ls:0};
  Object.values(shards).forEach(k=>{ const sh=SHARDS.find(s=>s.key===k);
    if(sh) for(const [st,v] of Object.entries(sh.stat)) a[st]=(a[st]||0)+v; });
  return a;
}
function stackStats(champ,n){
  champ = champ!==undefined ? champ : state.myChamp;
  n = n!==undefined ? n : (state.myStacks||0);
  const c=champByName(champ); const a={ad:0,ap:0,hp:0,armor:0,mr:0,crit:0};
  if(c&&c.stack){ for(const [st,v] of Object.entries(c.stack.per)) a[st]=(a[st]||0)+v*n; }
  return a;
}
/* суммарный бонус атакующего от осколков+стаков (для единого билда) */
function attackerBonus(){ const sh=shardStats(), st=stackStats();
  return { ad:sh.ad+st.ad, ap:sh.ap+st.ap, crit:sh.crit+st.crit, hp:sh.hp+st.hp, armor:sh.armor+st.armor, mr:sh.mr+st.mr, ls:sh.ls||0 }; }
function buildStats(){
  const c=champByName(state.myChamp), lvl=state.myLvl;
  ...
  const g=aggregateList(state.myItems,state.myBoots), sh=shardStats(), st=stackStats();
  const ks=keystoneStat(state.myKeystone, c?champDmgType(c):'ad');   // Завоеватель
  return {
    ...
    cost:gearGold(state.myItems,state.myBoots),
  };
}
...
function shardsHTML(){
  return [['Слот 1',0],['Слот 2',1],['Слот 3',2]].map(([label,row])=>{
    const opts=SHARDS.filter(s=>s.row===row), cur=state.myShards[row];
    return `<div class="dc-shardrow"><div class="dc-shardlbl">${label}</div>
      <div class="dc-runerow">${opts.map(o=>`<button class="dc-runepick ${cur===o.key?'on':''}" data-shard="${row}:${o.key}" title="${o.name} — ${o.note}">${o.img?runeImg(o.img):shardShort(o)}</button>`).join('')}</div></div>`;
  }).join('');
}
function buildBlock(){
  const c=champByName(state.myChamp);
  ...
  const stacksCol = (c&&c.stack) ? `<div class="dc-bcol glassy"><div class="dc-bcol-t">🔢 Стаки</div>
      <div class="dc-flabel">${c.stack.label} <span class="auto" id="bStacksLbl">${Math.min(state.myStacks,c.stack.max)} / ${c.stack.max}</span></div>
      <input type="range" id="bStacks" min="0" max="${c.stack.max}" value="${Math.min(state.myStacks,c.stack.max)}" style="width:100%;accent-color:var(--accent)">
  ...
  return `<div class="dc-build">
    ...
    <div class="dc-build-actions">
      <button class="dc-loadbtn" data-tobuild>⚔ Открыть «Калькулятор урона» с этим билдом</button>
      <button class="dc-sharebtn" data-buildshare>🔗 Поделиться билдом</button>
      <button class="dc-sharebtn ghost" data-buildimport>📥 Вставить код</button>
    </div>
  </div>`;
}
...
function applyBuildCode(code){
  try{
    ...
    const b = JSON.parse(decodeURIComponent(escape(atob(raw.trim()))));
    if(b.c!=null) state.myChamp=b.c;
...
function loadBuildToCalc(){
  // билд = атакующий (единый), осколки/стаки уже учтены в compute/fighterStats — просто открыть Калькулятор
...
function itemCalcBlock(){
  ...
  return `<div class="dc-itemcalc glassy">
    ...
    ${it.tick?`<div class="dc-bsim" id="dcDotCombat">${dotCombatHTML()}</div>`:''}
  </div>`;
}
...
/* ── Тик-симуляция DoT (Лиандри / Солнечный огонь): урон за 1 секунду по цели ── */
let dot={ start:0, cur:0, n:0, log:[] };
function dotReset(){ dot={ start:0, cur:0, n:0, log:[] }; }
function dotGV(id){ const e=document.getElementById(id); return e?+e.value||0:0; }
function dotTick(){
  const it=ITEMS[state.curItem]; if(!it||!it.perTick) return;
  const pt=it.perTick(dotGV);
  if(dot.n===0) dot.start=dot.cur=pt.startHp;
  if(dot.cur<=0){ renderDotCombat(); return; }
  dot.cur=Math.max(0,dot.cur-pt.dmg); dot.n++;
  dot.log.push({ n:dot.n, dmg:Math.round(pt.dmg), hp:Math.round(dot.cur) });
  renderDotCombat();
}
function dotCombatHTML(){
  const it=ITEMS[state.curItem]; if(!it||!it.perTick) return '';
  const pct=dot.start>0?Math.max(0,dot.cur/dot.start*100):100;
  const color=pct>60?'#2ecc71':pct>30?'#f1c40f':'#e74c3c';
  const dead=dot.n>0&&dot.cur<=0;
  const ctrl=`<div class="dc-irange dc-bbtns"><button data-dot="tick">⏱ Тик (1 сек)</button><button data-dot="reset">↺ Сброс</button></div>`;
  if(!dot.n) return ctrl+`<div class="dc-bhint">Жми «Тик» — каждый тик = урон за 1 секунду горения/ожога. Видно, за сколько секунд цель сгорит.</div>`;
  return ctrl+`
    <div class="dc-bhpbar"><div class="dc-bhpfill" style="width:${pct.toFixed(1)}%;background:${color}"></div></div>
    <div class="dc-bmeta"><span><b>${Math.round(dot.cur)}</b> / ${dot.start} HP</span><span>⏱ ${dot.n} сек</span></div>
    <div class="dc-blog">${dot.log.map(e=>`<div class="dc-blog-row"><span>Тик #${e.n} · ${e.n}с</span><span class="d">−${e.dmg}</span><span>${e.hp} HP</span></div>`).join('')}
      ${dead?`<div class="dc-bdead">🔥 СГОРЕЛ за ${dot.n} сек</div>`:''}</div>`;
}
function renderDotCombat(){
  const box=document.getElementById('dcDotCombat'); if(!box) return;
  box.innerHTML=dotCombatHTML();
  const h=box.querySelector('[data-dot="tick"]'); if(h)h.onclick=dotTick;
  const r=box.querySelector('[data-dot="reset"]'); if(r)r.onclick=()=>{ dotReset(); renderDotCombat(); };
}
...
function fighterStats(side){
  const isMy=side==='my';
  const ab=isMy?attackerBonus():null;   // осколки+стаки в единый билд
  return mkFighter(isMy
    ? {champ:state.myChamp,lvl:state.myLvl,items:state.myItems,boots:state.myBoots,baseAD:state.myBaseAD,keystone:state.myKeystone,
       bonusAD:(+state.myBonusAD||0)+ab.ad,ap:(+state.myAP||0)+ab.ap,crit:(+state.myCrit||0)+ab.crit,hpBonus:ab.hp,arBonus:ab.armor,mrBonus:ab.mr,
...
/* снимок текущего билда (для сравнения «B») */
function snapshotBuild(){ return { champ:state.myChamp, lvl:state.myLvl, items:state.myItems.slice(), boots:state.myBoots,
  keystone:state.myKeystone, shards:Object.assign({},state.myShards), stacks:state.myStacks }; }
/* боец из снимка билда (осколки+стаки сворачиваются) */
function fighterFromSnap(s){ const sh=shardStats(s.shards), st=stackStats(s.champ,s.stacks);
  return mkFighter({champ:s.champ,lvl:s.lvl,items:s.items,boots:s.boots,keystone:s.keystone,
...
const QUIZ_N=20;
/* банк ФАКТ-вопросов (механики/предметы/руны WR — стабильные, по данным из tooltip'ов). {q, ok:верный, wr:[3 неверных], note} */
const QFACTS=[
  {q:'На сколько % снижает входящий физ. урон 100 брони?', ok:'на 50%', wr:['на 100%','на 25%','на 75%'], note:'100/(100+100)=50%'},
  {q:'Какой урон игнорирует и броню, и сопротивление магии?', ok:'Чистый (true)', wr:['Физический','Магический','Смешанный'], note:'чистый урон не митигируется'},
  {q:'Какой предмет наносит он-хит % от ТЕКУЩЕГО HP цели?', ok:'Клинок Погибшего Короля', wr:['Грань Бесконечности','Шипованный доспех','Зуб Нашора'], note:'BotRK: 10%/7% тек. HP'},
  {q:'Что делает «Тяжкие раны»?', ok:'Снижают лечение цели', wr:['Снижают броню','Замедляют','Жгут по % HP'], note:'обычно −60% к лечению'},
  {q:'Против каких целей выгоднее всего «Мучения Лиандри»?', ok:'С большим запасом HP (танки)', wr:['Хрупкие ассасины','Цели без маны','Дальний бой'], note:'ожог считает % макс. HP'},
  {q:'Как складывается процентное пробивание брони от нескольких источников?', ok:'Мультипликативно', wr:['Просто суммируется','Берётся максимум','Не складывается'], note:'(1−a)(1−b)…'},
  {q:'Какие статы даёт «Грань Бесконечности»?', ok:'AD + шанс крита', wr:['AP + мана','Броня + HP','Скор.атаки + вампиризм'], note:'+урон от критов'},
  {q:'Базовая броня снижает урон какого типа?', ok:'Физический', wr:['Магический','Чистый','Любой'], note:'от магии защищает МС'},
  {q:'Что копит руна «Завоеватель» в затяжном бою?', ok:'Адаптивную силу (AD/AP)', wr:['Броню','Скорость передвижения','Ману'], note:'стаки на ударах'},
  {q:'Что разгоняет руна «Смертельный темп»?', ok:'Скорость атаки', wr:['Силу умений','Пробивание брони','Лечение'], note:'стаки на авто-атаках'},
  {q:'Что снижает «Чёрный тесак» при ударах?', ok:'Броню цели (стаками)', wr:['Сопротивление магии','Скорость атаки','Лечение'], note:'до −24% брони'},
  {q:'Какой предмет даёт магический он-хит от скорости атаки и AP?', ok:'Зуб Нашора', wr:['Жажда крови','Шипованный доспех','Сила природы'], note:'on-hit маг. урон'},
  {q:'Отрицательная (минусовая) броня цели делает что?', ok:'Усиливает входящий физ. урон', wr:['Ничего, считается как 0','Лечит цель','Снижает её скорость'], note:'2−100/(100−|бр|)'},
  {q:'Крит-удар по умолчанию наносит сколько от обычного?', ok:'около ×2', wr:['×1.5','×3','×1.2'], note:'+бонус от Грани Бесконечности'},
  {q:'Что даёт «Шипованный доспех» при получении авто-атак?', ok:'Отражает магический урон', wr:['Лечит владельца','Оглушает','Ускоряет'], note:'+ Тяжкие раны'},
  {q:'Какая защита спасает от урона способностей мага?', ok:'Сопротивление магии', wr:['Броня','HP-реген','Скор.атаки'], note:'МС снижает маг. урон'},
];
...
let quiz=null, quizTimerInt=null;
/* конструктор своих квизов: makeView 'list'|'edit', userQuizDraft = редактируемый набор */
let makeView='list', userQuizDraft=null, makeMsg='';
...
// ФАКТ-вопрос (4 варианта, перемешанные)
function qFact(){
  const f=QFACTS[rint(0,QFACTS.length-1)];
  const opts=[{label:f.ok,_c:true},...f.wr.map(w=>({label:w}))];
  for(let i=opts.length-1;i>0;i--){ const j=rint(0,i); [opts[i],opts[j]]=[opts[j],opts[i]]; }
  return { kind:'fact', q:f.q, opts, correct:opts.findIndex(o=>o._c), hint:f.note||'' };
}
// карточки для сценария «убьёт ли»
function killCardsHTML(A,ac,alvl,aitems,atype,ks, B,tc,tlvl,titems){
  const aIco=aitems.map(k=>GEAR[k]?itemImg(GEAR[k].img):'').join('') + (KEYSTONES[ks]&&KEYSTONES[ks].img?runeImg(KEYSTONES[ks].img):'');
  const tIco=titems.map(k=>GEAR[k]?itemImg(GEAR[k].img):'').join('');
  return `<div class="dc-guess-row">
    <div class="dc-guess-side"><div class="dc-guess-t">${atype==='ap'?'AP':'AD'} · бьёт</div>${fAva2(ac.name,ac.dd)}<b>${ac.name}</b><small>ур.${alvl} · AD ${Math.round(A.ad)} · AP ${Math.round(A.ap)}</small><div class="dc-guess-items">${aIco}</div></div>
    <div class="dc-guess-combo">⚔→</div>
    <div class="dc-guess-side"><div class="dc-guess-t">цель</div>${fAva2(tc.name,tc.dd)}<b>${tc.name}</b><small>ур.${tlvl}</small><div class="dc-guess-items">${tIco}</div><div class="dc-guess-def">${Math.round(B.hp)} HP · ${B.armor|0} бр · ${B.mr|0} мс</div></div>
  </div>`;
}
// РАСЧЁТ-сценарий «убьёт ли комбо» (4 варианта по соотношению урон/HP, роле-корректные билды)
function qKill(){
  let c; do{ c=CHAMPS[rint(0,CHAMPS.length-1)]; }while(!c.abils||c.abils.length<2);
  const type=champDmgType(c), lvl=rint(11,15);
  const items=pickN(type==='ap'?AP_ITEMS:AD_ITEMS, rint(2,4));
  const ks=type==='ap'?'electrocute':'conqueror';
  const A=mkFighter({champ:c.name,lvl,items,keystone:ks});
  const tg=CHAMPS[rint(0,CHAMPS.length-1)], tItems=pickN(DEF_ITEMS,rint(1,3)), tlvl=rint(11,15);
  const B=mkFighter({champ:tg.name,lvl:tlvl,items:tItems});
  const dmg=comboTotalKS(A,B), ratio=B.hp>0?dmg/B.hp:0;
  const correct = ratio>=1.25?0 : ratio>=1.0?1 : ratio>=0.8?2 : 3;
  return { kind:'kill', q:`Хватит ли полного комбо «${c.name}», чтобы убить «${tg.name}» (полное HP)?`,
    ctx:killCardsHTML(A,c,lvl,items,type,ks, B,tg,tlvl,tItems),
    opts:[{label:'Да, с запасом'},{label:'Да, впритык'},{label:'Нет, чуть не хватит'},{label:'Нет, далеко'}],
    correct, hint:`урон комбо ${Math.round(dmg)} vs ${Math.round(B.hp)} HP (${Math.round(ratio*100)}%)` };
}
// микс типов: факты / расчёт-килл / относительные (с отбраковкой ничьих)
function quizGen(){
  const r=Math.random();
  if(r<0.40) return qFact();
  if(r<0.70) return qKill();
  const gens=[qAbil,qItem,qTarget];
  for(let tries=0;tries<40;tries++){ const q=gens[rint(0,gens.length-1)]();
    if(Math.abs(q.d1-q.d2) >= Math.max(q.d1,q.d2)*0.05 + 8) return q; }
  return qFact();
}
const QUIZ_SEC=20;   // секунд на вопрос
function quizStart(){ const qs=[]; for(let i=0;i<QUIZ_N;i++)qs.push(quizGen());
  quiz={ qs, n:QUIZ_N, i:0, score:0, correct:0, picked:null, qStart:performance.now(), deadline:performance.now()+QUIZ_SEC*1000, done:false, saved:false }; }
function quizAnswer(idx){
  ...
  q.ok=ok; q.sec=sec; q.timedout=(idx===-1);
  if(ok){ quiz.correct++; q.speed=Math.max(0,Math.round(50-sec*4)); quiz.score+=100+q.speed; }
}
function quizNext(){ if(quiz.i<quiz.n-1){ quiz.i++; quiz.picked=null; quiz.qStart=performance.now(); quiz.deadline=performance.now()+QUIZ_SEC*1000; } else quiz.done=true; }
function quizTick(){
  if(!quiz||quiz.done||quiz.picked!=null){ if(quizTimerInt){clearInterval(quizTimerInt);quizTimerInt=null;} return; }
  const rem=Math.max(0,(quiz.deadline-performance.now())/1000);
  const bar=document.getElementById('quizTimerBar'); if(bar){ bar.style.width=(rem/QUIZ_SEC*100)+'%'; bar.style.background=rem<5?'#ff6b6b':rem<10?'#f1c40f':'var(--accent)'; }
  const lbl=document.getElementById('quizTimerLbl'); if(lbl) lbl.textContent=Math.ceil(rem)+'с';
  if(rem<=0){ clearInterval(quizTimerInt); quizTimerInt=null; quizAnswer(-1); renderStage(); }
}
function quizLB(){ try{ return JSON.parse(localStorage.getItem('dc-quiz-lb')||'[]'); }catch(e){ return []; } }
function quizSave(name){ const lb=quizLB(); lb.push({name:(name||'Аноним').slice(0,16),score:quiz.score,correct:quiz.correct,n:quiz.n,date:Date.now()});
  lb.sort((a,b)=>b.score-a.score); localStorage.setItem('dc-quiz-lb',JSON.stringify(lb.slice(0,20))); quiz.saved=true; }
function quizHTML(){
  if(!quiz) quizStart();
  if(quiz.done){
    // кастомный квизз юзера — без общего рейтинга (разная длина/сложность)
    if(quiz.custom){
      return `<div class="dc-arena-mode"><div class="dc-quiz-end glassy">
        <div class="dc-quiz-end-h">Квизз «${quiz.custom}» пройден!</div>
        <div class="dc-quiz-score"><span class="v">${quiz.correct} / ${quiz.n}</span><span class="l">верно · <b>${quiz.score}</b> очков</span></div>
        <div class="dc-arena-ctrl"><button class="dc-playbtn" data-quiz="restart">↻ Ещё раз</button><button class="dc-resetbtn" data-mk="list">← К моим квизам</button></div>
      </div></div>`;
    }
    const lb=quizLB();
    return `<div class="dc-arena-mode"><div class="dc-quiz-end glassy">
      ...
      <div class="dc-quiz-lb">${lb.length?lb.slice(0,8).map((e,i)=>`<div class="dc-quiz-lb-row"><span class="rk">${i+1}</span><span class="nm">${e.name}</span><span>${e.correct}/${e.n||QUIZ_N}</span><span class="sc">${e.score}</span></div>`).join(''):'<div class="dc-gmuted">пока пусто — будь первым</div>'}</div>
  ...
  const kindTag = quiz.custom?'✍ Свой' : q.kind==='fact'?'📚 Факт' : q.kind==='kill'?'🧮 Расчёт' : '⚖ Сравнение';
  return `<div class="dc-arena-mode">
    ${quiz.custom?`<div class="dc-arena-hint">Свой квизз: <b>${quiz.custom}</b></div>`:''}
    <div class="dc-quiz-top"><span>Вопрос <b>${quiz.i+1}</b> / ${quiz.n} · <span class="dc-quiz-kind">${kindTag}</span></span><span>Очки: <b>${quiz.score}</b> · <span id="quizTimerLbl">${QUIZ_SEC}с</span></span></div>
    <div class="dc-quiz-timer"><span id="quizTimerBar" style="width:100%"></span></div>
    <div class="dc-guess glassy">
      ${q.ctx||''}
      <div class="dc-quiz-q">${q.q}</div>
      <div class="dc-quiz-opts ${q.opts.length>2?'four':''}">${q.opts.map((o,idx)=>{ let cls=''; if(answered){ if(idx===q.correct)cls='right'; else if(idx===quiz.picked)cls='wrong'; }
      ...
      ${answered?`<div class="dc-quiz-fb ${q.ok?'ok':'no'}">${q.ok?`✓ Верно! +${100+(q.speed||0)} очк.`:(q.timedout?'⏱ Время вышло':'✗ Мимо')} <span class="dc-quiz-hint">${q.hint||''}</span></div>
        <div class="dc-arena-ctrl"><button class="dc-playbtn" data-quiz="next">${quiz.i<quiz.n-1?'→ Дальше':'Итог →'}</button></div>`:''}
    </div>
  </div>`;
}

/* ════════════ ✍ СВОЙ КВИЗЗ (конструктор + менеджер квизов юзера) ════════════
   Хранение в localStorage (dc-user-quizzes). Вопросы — факт-типа (текст + до 4 вариантов
   + верный + подсказка), играются тем же движком (kind:'fact'). «Спросить разрешение» =
   отправка на модерацию (status:'submitted') — на боевом уйдёт в CMS/админку. */
function userQuizzes(){ try{ return JSON.parse(localStorage.getItem('dc-user-quizzes')||'[]'); }catch(e){ return []; } }
...
function draftAddQuestion(box){
  const q=(box.querySelector('#mkQ').value||'').trim();
  const opts=[0,1,2,3].map(i=>(box.querySelector('#mkO'+i).value||'').trim());
  const filled=opts.map((label,i)=>({label,i})).filter(o=>o.label);
  const corRadio=box.querySelector('input[name=mkCorrect]:checked');
  const cor=corRadio?+corRadio.value:0;
  const hint=(box.querySelector('#mkHint').value||'').trim();
  if(!q) return {err:'Впиши текст вопроса'};
  if(filled.length<2) return {err:'Нужно минимум 2 варианта ответа'};
  if(!opts[cor]) return {err:'Отметь как верный ЗАПОЛНЕННЫЙ вариант'};
  // пересобрать индекс верного среди заполненных
  const newOpts=filled.map(o=>({label:o.label}));
  const newCor=filled.findIndex(o=>o.i===cor);
  userQuizDraft.questions.push({ q, opts:newOpts, correct:newCor, hint });
  return {ok:true};
}

function makeListHTML(){
  const list=userQuizzes();
  const rows=list.length? list.map(qz=>`<div class="dc-uq-row glassy">
      <div class="dc-uq-info"><b>${qz.name||'(без названия)'}</b>
        <small>${qz.questions.length} вопр.${qz.author?' · '+qz.author:''} · ${qz.status==='submitted'?'<span class="dc-uq-badge sub">📨 на модерации</span>':'<span class="dc-uq-badge">черновик</span>'}</small></div>
      <div class="dc-uq-acts">
        <button class="dc-playbtn sm" data-mk="play:${qz.id}" ${qz.questions.length?'':'disabled'}>▶ Играть</button>
        <button class="dc-resetbtn" data-mk="edit:${qz.id}">✍ Изменить</button>
        ${qz.status==='submitted'?'':`<button class="dc-resetbtn" data-mk="ask:${qz.id}" ${qz.questions.length?'':'disabled'}>📨 Спросить разрешение</button>`}
        <button class="dc-resetbtn" data-mk="del:${qz.id}">🗑</button>
      </div>
    </div>`).join('') : '<div class="dc-gmuted" style="padding:14px 0">Пока нет своих квизов — создай первый.</div>';
  return `<div class="dc-arena-mode">
    <div class="dc-uq-banner glassy">
      <div class="dc-uq-banner-h">✍ Сделай свой квизз</div>
      <div class="dc-uq-banner-t">Собери свои вопросы (текст + 4 варианта + верный) и сыграй сразу. Хочешь, чтобы твой квизз попал в <b>общую</b> викторину сайта — жми <b>«Спросить разрешение»</b>: он уйдёт на модерацию.</div>
    </div>
    ${makeMsg?`<div class="dc-uq-msg">${makeMsg}</div>`:''}
    <div class="dc-arena-ctrl"><button class="dc-playbtn" data-mk="new">➕ Новый квизз</button></div>
    <div class="dc-uq-list">${rows}</div>
  </div>`;
}

function makeEditHTML(){
  const d=userQuizDraft;
  const qlist=d.questions.length? d.questions.map((Q,i)=>`<div class="dc-uq-qrow glassy">
      <span class="dc-uq-qn">${i+1}</span>
      <div class="dc-uq-qbody"><b>${Q.q}</b><small>✓ ${Q.opts[Q.correct].label}${Q.opts.length>1?' · из '+Q.opts.length+' вар.':''}</small></div>
      <button class="dc-resetbtn" data-mk="delq:${i}">🗑</button>
    </div>`).join('') : '<div class="dc-gmuted">Вопросов пока нет — добавь ниже.</div>';
  const radios=[0,1,2,3].map(i=>`<label class="dc-uq-radio"><input type="radio" name="mkCorrect" value="${i}" ${i===0?'checked':''}><input class="dc-input" id="mkO${i}" type="text" maxlength="60" placeholder="вариант ${i+1}${i<2?' (обяз.)':''}"></label>`).join('');
  return `<div class="dc-arena-mode">
    ${makeMsg?`<div class="dc-uq-msg">${makeMsg}</div>`:''}
    <div class="dc-uq-meta glassy">
      <div class="dc-field"><label class="dc-flabel">Название квизза</label><input class="dc-input" id="mkName" type="text" maxlength="40" value="${(d.name||'').replace(/"/g,'&quot;')}" placeholder="напр. «Предметы для новичков»"></div>
      <div class="dc-field"><label class="dc-flabel">Автор (ник)</label><input class="dc-input" id="mkAuthor" type="text" maxlength="16" value="${(d.author||'').replace(/"/g,'&quot;')}" placeholder="как тебя показать"></div>
    </div>
    <div class="dc-uq-qhead">📝 Вопросы (${d.questions.length})</div>
    <div class="dc-uq-qs">${qlist}</div>
    <div class="dc-uq-form glassy">
      <div class="dc-uq-form-h">➕ Новый вопрос</div>
      <div class="dc-field"><label class="dc-flabel">Текст вопроса</label><input class="dc-input" id="mkQ" type="text" maxlength="120" placeholder="напр. Что снижает «Чёрный тесак»?"></div>
      <div class="dc-flabel" style="margin-top:6px">Варианты ответа (отметь точкой верный):</div>
      <div class="dc-uq-radios">${radios}</div>
      <div class="dc-field"><label class="dc-flabel">Подсказка/пояснение (необязательно)</label><input class="dc-input" id="mkHint" type="text" maxlength="80" placeholder="покажется после ответа"></div>
      <div class="dc-arena-ctrl"><button class="dc-playbtn sm" data-mk="addq">＋ Добавить вопрос</button></div>
    </div>
    <div class="dc-arena-ctrl">
      <button class="dc-playbtn" data-mk="save" ${d.questions.length?'':'disabled'}>💾 Сохранить квизз</button>
      <button class="dc-playbtn sm" data-mk="playdraft" ${d.questions.length?'':'disabled'}>▶ Проверить (сыграть)</button>
      <button class="dc-resetbtn" data-mk="cancel">← Отмена</button>
    </div>
  </div>`;
}

function makeHTML(){ return makeView==='edit'&&userQuizDraft ? makeEditHTML() : makeListHTML(); }

/* обработчик действий конструктора (data-mk) */
function handleMake(v,box){
  makeMsg='';
  if(v==='new'){ userQuizDraft=newDraft(); makeView='edit'; renderStage(); return; }
  if(v==='cancel'){ userQuizDraft=null; makeView='list'; renderStage(); return; }
  if(v==='list'){ makeView='list'; state.arena='make'; renderStage(); return; }
  if(v==='addq'){ const r=draftAddQuestion(box); makeMsg=r.ok?'✓ Вопрос добавлен':('⚠ '+r.err); renderStage(); return; }
  if(v.startsWith('delq:')){ userQuizDraft.questions.splice(+v.slice(5),1); renderStage(); return; }
  if(v==='save'){
    const nm=(box.querySelector('#mkName')?.value||'').trim();
    userQuizDraft.name=nm||('Квизз '+new Date().toLocaleDateString('ru'));
    userQuizDraft.author=(box.querySelector('#mkAuthor')?.value||'').trim();
    ...
    saveUserQuizzes(all); userQuizDraft=null; makeView='list'; makeMsg='✓ Квизз сохранён'; renderStage(); return;
  }
  if(v==='playdraft'){
  ...
  if(v.startsWith('del:')){ saveUserQuizzes(userQuizzes().filter(q=>q.id!==v.slice(4))); renderStage(); return; }
  if(v.startsWith('ask:')){ const all=userQuizzes(); const qz=all.find(q=>q.id===v.slice(4));
    if(qz){ qz.status='submitted'; qz.submittedAt=Date.now(); saveUserQuizzes(all);
      makeMsg='📨 Заявка отправлена на модерацию. После проверки квизз появится в общей викторине.'; }
    renderStage(); return; }
}
...
function arenaBlock(){
  ...
  else if(state.arena==='make') body=makeHTML();
  else body=duelHTML();
...
function autoInit(){ const A=fighterStats('my'), B=fighterStats('tgt');
  auto={ A, B, cur:B.hp, hits:0, t:0, dmg:0, lt:0, conq:0, last:0, running:false, dead:false,
         src:{}, timeline:[], dmgAt3s:0, healed:0 }; }
...
function autoHit(){
  ...
  const mPhys=mitFromEff((B.armor>0?B.armor*A.arFrac:B.armor)-A.arFlat);
  const mMag =mitFromEff((B.mr>0?B.mr*A.mrFrac:B.mr)-A.mrFlat);
  ...
  const addSrc=(k,v)=>{ if(v>0) auto.src[k]=(auto.src[k]||0)+v; };
  let dmg=0;
  const baseAA=ad*mPhys; dmg+=baseAA; addSrc('Авто',baseAA);                       // базовый авто
  if(crit){ dmg+=baseAA; addSrc('Крит',baseAA); }                                   // бонус крита
  (state.myItems||[]).forEach(k=>{ const it=GEAR[k]; if(it&&it.onhit){ const o=it.onhit;
    ...
    const m=o.dt==='magic'?mMag:o.dt==='true'?1:mPhys; const v=raw*m; dmg+=v; addSrc(it.name,v); } });
  if(A.keystone==='lethaltempo' && auto.lt>=6){ const v=(20+0.15*ad)*mMag; dmg+=v; addSrc('Руна (Темп)',v); }
  auto.cur=Math.max(0,auto.cur-dmg); auto.dmg+=dmg; auto.hits++; auto.last=dmg;
  if(A.ls>0) auto.healed += dmg*A.ls/100;                                            // вампиризм
  ...
  auto.timeline.push({ t:auto.t, cum:auto.dmg });
  if(auto.t<=3.0001) auto.dmgAt3s=auto.dmg;                                          // бёрст за первые 3с
  autoRenderLive();
...
function autoRenderLive(){
  ...
  const bk=document.getElementById('autoBreak'); if(bk) bk.innerHTML=autoBreakHTML();
}
function autoSpark(){
  const tl=auto.timeline; if(tl.length<2) return '';
  const maxT=tl[tl.length-1].t||1, maxD=tl[tl.length-1].cum||1, W=320,H=46;
  const pts=tl.map(p=>`${(p.t/maxT*W).toFixed(1)},${(H-p.cum/maxD*(H-3)).toFixed(1)}`).join(' ');
  return `<svg class="dc-auto-spark" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none"><polyline points="${pts}" fill="none" stroke="var(--accent)" stroke-width="2"/></svg>`;
}
function autoBreakHTML(){
  if(!auto||auto.hits===0) return '';
  const sustained=Math.round(auto.dmg/Math.max(auto.t,0.01));
  const burst=Math.round(auto.dmgAt3s/Math.min(auto.t||3,3));
  const ent=Object.entries(auto.src).sort((a,b)=>b[1]-a[1]);
  const max=Math.max(1,...ent.map(e=>e[1]));
  return `<div class="dc-auto-dpsrow">
      <span>📈 Устоявшийся DPS: <b>${sustained}</b></span>
      <span>⚡ Бёрст (3с): <b>${burst}</b></span>
      ${auto.healed>1?`<span class="hl">💚 +${Math.round(auto.healed)} HP вампиризм</span>`:''}
    </div>
    ${autoSpark()}
    <div class="dc-auto-break">
      <div class="dc-auto-break-h">Откуда урон</div>
      ${ent.map(([k,v])=>`<div class="dc-auto-brow"><span class="nm">${k}</span><span class="bar"><span style="width:${v/max*100}%"></span></span><span class="v">${Math.round(v)}</span><span class="pc">${Math.round(v/auto.dmg*100)}%</span></div>`).join('')}
    </div>`;
}
function autoBlock(){
  if(!auto) autoInit();
...
function compareBlock(){
  ...
  const snapB = state.cmpB;
  const sides=[
    {tag:'A · Текущий билд', F:fighterStats('my'), items:state.myItems, boots:state.myBoots, ks:state.myKeystone},
    snapB
      ? {tag:'B · Сохранённый', F:fighterFromSnap(snapB), items:snapB.items, boots:snapB.boots, ks:snapB.keystone}
      : {tag:'B · (пусто)', F:fighterStats('my'), items:state.myItems, boots:state.myBoots, ks:state.myKeystone},
  ];
  ...
  return `<div class="dc-cmp">
    <div class="dc-arena-hint">A = твой текущий билд (из Сборки/Урона), B = сохранённый снимок. Цель «${T.name}» (${Math.round(T.hp)} HP · ${T.armor|0} бр · ${T.mr|0} мс). Нажми «Запомнить как B», измени билд — и сравни два варианта. Зелёным — кто лучше по метрике.</div>
    <div class="dc-arena-ctrl"><button class="dc-playbtn" data-cmpsave>📌 Запомнить текущий билд как B</button>${snapB?'<button class="dc-resetbtn" data-cmpclear>↺ Очистить B</button>':''}</div>
    <div class="dc-cmp-grid">${col(data[0],0)}${col(data[1],1)}</div>
  </div>`;
}

/* ════════════ ОПТИМИЗАТОР «ЧТО КУПИТЬ ПРОТИВ ЦЕЛИ» ════════════
   Перебирает предметы движком урона, ранжирует по приросту комбо против текущей цели.
   Свободный слот → добавления; полная сборка (6) → лучшие замены. Чистый расчёт (comboTotalKS). */
function optimizeBuild(){
  const B = fighterStats('tgt');
  const have = state.myItems.slice();
  const saveI = state.myItems;
  const dmgWith = items => { state.myItems = items; const A = fighterStats('my'); const d = comboTotalKS(A,B); state.myItems = saveI; return d; };
  const base = dmgWith(have);
  const pool = Object.keys(GEAR).filter(k => !have.includes(k));
  const res = [];
  if(have.length < 6){
    pool.forEach(k => { const d = dmgWith(have.concat(k)); res.push({ key:k, gain:d-base, pct:base>0?(d-base)/base*100:0, cost:GOLD[k]||3000 }); });
  } else {
    have.forEach((cur,i) => pool.forEach(k => { const arr=have.slice(); arr[i]=k; const d=dmgWith(arr);
      res.push({ key:k, replace:cur, gain:d-base, pct:base>0?(d-base)/base*100:0, cost:GOLD[k]||3000 }); }));
  }
  res.sort((a,b)=>b.gain-a.gain);
  return { base, list:res.slice(0,6), full:have.length>=6 };
}
function optimPanelHTML(){
  if(state.myItems.length===0 && state.myBoots==null) {} // ок, считаем и от пустого
  const o = optimizeBuild();
  if(!o.list.length) return `<div class="dc-optim"><div class="dc-optim-empty">Нет кандидатов</div></div>`;
  return `<div class="dc-optim">
    <div class="dc-optim-h">${o.full?'⚔ Лучшие ЗАМЕНЫ против цели':'💰 Что добавить — топ по приросту урона по цели'}</div>
    ${o.list.map(s=>`<button class="dc-optim-row" data-optadd="${s.key}${s.replace?(':'+s.replace):''}">
      ${itemImg(GEAR[s.key].img)}
      <span class="dc-optim-info"><b>${GEAR[s.key].name}</b>${s.replace?`<small>вместо «${GEAR[s.replace].name}»</small>`:''}</span>
      <span class="dc-optim-gain"><span class="g">+${Math.round(s.gain)}</span><span class="p">+${s.pct.toFixed(0)}%</span></span>
      <span class="dc-optim-cost">🪙 ${s.cost}</span>
    </button>`).join('')}
    <div class="dc-optim-note">Считается комбо против текущей цели · нажми вариант — добавится в билд</div>
  </div>`;
}
...
function wireModal(){
  ...
  inner.querySelectorAll('.dc-tab').forEach(t=>t.addEventListener('click',()=>{ if(!t.dataset.tab)return; state.tab=t.dataset.tab; gearPick=null; renderStage(); renderTabSettings(); }));
  ...
  if(bStacks) bStacks.oninput=e=>{ state.myStacks=+e.target.value; const lbl=inner.querySelector('#bStacksLbl'); const c=champByName(state.myChamp);
    if(lbl&&c&&c.stack)lbl.textContent=state.myStacks+' / '+c.stack.max;
    const sheet=inner.querySelector('#dcStatSheet'); if(sheet)sheet.innerHTML=statSheetHTML(buildStats()); };
  inner.querySelectorAll('[data-shard]').forEach(b=>b.addEventListener('click',()=>{ const [row,key]=b.dataset.shard.split(':'); state.myShards[row]=key; renderStage(); }));
  const tobuild=inner.querySelector('[data-tobuild]'); if(tobuild) tobuild.onclick=loadBuildToCalc;
  const cmpS=inner.querySelector('[data-cmpsave]'); if(cmpS) cmpS.onclick=()=>{ state.cmpB=snapshotBuild(); renderStage(); };
  const cmpC=inner.querySelector('[data-cmpclear]'); if(cmpC) cmpC.onclick=()=>{ state.cmpB=null; renderStage(); };
  const bshare=inner.querySelector('[data-buildshare]');
  if(bshare) bshare.onclick=()=>{ const url=location.href.split('#')[0]+'#b='+serializeBuild();
    const done=()=>{ bshare.textContent='✓ Ссылка скопирована!'; setTimeout(()=>{bshare.textContent='🔗 Поделиться билдом';},1700); };
    if(navigator.clipboard&&navigator.clipboard.writeText) navigator.clipboard.writeText(url).then(done,()=>{ window.prompt('Скопируй ссылку на билд:',url); });
    else window.prompt('Скопируй ссылку на билд:',url); };
  const bimport=inner.querySelector('[data-buildimport]');
  if(bimport) bimport.onclick=()=>{ const code=window.prompt('Вставь код или ссылку билда:'); if(!code) return;
    if(applyBuildCode(code)){ syncAutoStats&&syncAutoStats(); renderStage(); } else window.alert('Не удалось разобрать код билда'); };
  ...
  inner.querySelectorAll('[data-quiz]').forEach(b=>b.addEventListener('click',()=>{ const v=b.dataset.quiz;
    ...
    else if(v==='restart'){ if(quiz&&quiz.customId) playUserQuiz(quiz.customId); else { quizStart(); renderStage(); } }
    else if(v==='save'){ const n=inner.querySelector('#quizName'); quizSave(n?n.value.trim():''); renderStage(); } }));
  // ✍ конструктор своих квизов
  inner.querySelectorAll('[data-mk]').forEach(b=>b.addEventListener('click',()=>handleMake(b.dataset.mk, inner)));
  // таймер 20с на вопрос
  if(state.tab==='arena' && state.arena==='guess' && quiz && !quiz.done && quiz.picked==null){
    if(quizTimerInt) clearInterval(quizTimerInt);
    quizTimerInt=setInterval(quizTick,250); quizTick();
  }
  ...
  inner.querySelectorAll('[data-rune]').forEach(sel=>sel.addEventListener('change',e=>{
    if(sel.dataset.rune==='my') state.myKeystone=e.target.value; else state.myKeystone=e.target.value; renderStage(); }));
...
function wireItemCalc(){
  ...
  inner.querySelectorAll('.dc-itemcard').forEach(c=>c.addEventListener('click',()=>{
    state.curItem=c.dataset.item; state._itemFields={}; botrkReset(); dotReset();
  ...
  // BotRK / DoT: забиндить кнопки симуляции
  if(ITEMS[state.curItem]&&ITEMS[state.curItem].combat) renderBotrkCombat();
  if(ITEMS[state.curItem]&&ITEMS[state.curItem].tick) renderDotCombat();
}
...
function group(label,tab){ return `<div class="ls-group">${label}${tab?`<span class="ls-group-tab">${TAB_NAMES[tab]||tab}</span>`:''}</div>`; }
function renderSettings(){
  const s=document.getElementById('settings');
  s.innerHTML =
    group('🎨 Общий вид (везде)')+
    ...
    `<div id="tabSettings" style="display:contents"></div>`;
...
/* ── динамическая часть полосы: настройки АКТИВНОЙ вкладки (size/design + доп.) ── */
function renderTabSettings(){
  const box=document.getElementById('tabSettings'); if(!box) return;
  const tab=state.tab, tc=tcfg(tab), extras=TAB_EXTRAS[tab]||[];
  let html=group('⚙ Настройки вкладки',tab)+
    selCtrl('t_size','Размер',TAB_SIZE,tc.size||'norm')+
    selCtrl('t_design','Дизайн',TAB_DESIGN,tc.design||'cards');
  ...
  // size/design только меняют data-атрибут на модалке — без полного рендера (живо, без потери фокуса)
  get('t_size').onchange=e=>{ tc.size=e.target.value; const m=document.querySelector('.dc-modal'); if(m)m.dataset.tsize=tc.size; };
  get('t_design').onchange=e=>{ tc.design=e.target.value; const m=document.querySelector('.dc-modal'); if(m)m.dataset.tdesign=tc.design; };
...
/* старт */
// билд из ссылки (#b=...) → открыть на вкладке Сборка
if(location.hash.indexOf('b=')>=0 && applyBuildCode(location.hash)) state.tab='build';
syncAutoStats();
ensureAbilState();
renderSettings();
renderStage();
...
if(window.LabSettings){
  LS=LabSettings.attach({
    id:'damage-calc', defaults:DEFAULTS, mount:'#labTools', schema:2,
    getState:()=>state,
