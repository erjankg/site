...
/* Зонтичные теги: собираются из точных, чтобы старые правила не переписывать.
   Подброс и притягивание сюда НЕ входят — у них свои правила с другими весами. */
const UMBRELLA = {
  cc_hard: ['cc_stun', 'cc_taunt', 'cc_suppress', 'cc_charm', 'cc_fear', 'cc_sleep', 'cc_polymorph'],
};

const RX = {
  /* ── контроль ──
     Раньше cc_hard сваливал в одну кучу восемь разных вещей: стан, насмешку,
     подавление, очарование, страх, сон, превращение, заморозку. Для драфта это
     РАЗНОЕ: насмешка ломает керри (бьёт не туда), подавление не снимается обычной
     чисткой, страх и очарование уводят цель с позиции. Теперь каждый вид своим
     тегом, а cc_hard остаётся зонтиком над ними — чтобы прежние правила и функции
     состава продолжали работать (см. UMBRELLA ниже). */
  cc_stun:      { re: /晕眩|眩晕|击晕|昏迷|冰冻|石化/ },
  cc_taunt:     { re: /嘲讽/ },
  cc_suppress:  { re: /压制/ },
  cc_charm:     { re: /魅惑/ },
  cc_fear:      { re: /恐惧/ },
  cc_sleep:     { re: /沉睡|昏睡/ },
  /* «变形» значит и «превратить врага» (Лулу), и «сменить свою форму» (Джейс,
     Нидали, Элиза). Отсекаем второе: у своей формы рядом стоят «оружие»,
     «форма», «облик» или сама пассивка про переключение. */
  cc_polymorph: { re: /变形/, no: /武器|形态|变身|切换|姿态|人形|兽形/ },
  ...
  /* ── ПРОТИВОДЕЙСТВИЕ КОНТРОЛЮ ──
     Раньше всё лежало под одним cc_immune, а это СЕМЬ разных механик, и они
     закрывают разное. Главное различие: стойкость только СОКРАЩАЕТ длительность
     и не действует ни на подброс, ни на подавление; щит от умений съедает целое
     умение вместе с его контролем (то есть может съесть вражескую ульту). */
  cc_immune:     { re: /免疫(所有)?控制|免疫控制效果/ },
  cc_parry:      { re: /抵挡.{0,12}控制效果|格挡.{0,12}控制/ },
  cc_spellshield:{ re: /法术护盾|格挡一次敌方技能|可格挡.{0,10}技能/ },
  cc_unstoppable:{ re: /不可阻挡/ },
  cc_cleanse:    { re: /移除.{0,10}控制效果|净化.{0,14}(限制|定身|控制)|解除.{0,8}(控制|减益)/ },
  cc_tenacity:   { re: /韧性/ },
  // старый зонтик: любая из антиконтрольных механик — чтобы прежние правила жили
  cc_immune_any: { re: /免疫(所有)?控制|不可阻挡|法术护盾|抵挡.{0,12}控制效果|移除.{0,10}控制效果|韧性/ },
  // «无法选中» без 被 — так написано у Гвен; промах на один иероглиф стоил ей тега,
  // а вместе с ним и всей связки «стена перед защищённым керри».
  untargetable: { re: /无法(被)?选中|不可(被)?选取|无法(被)?选取/ },
...
export const TAG_RU = {
  cc_stun: 'оглушение', cc_taunt: 'насмешка', cc_suppress: 'подавление',
  cc_charm: 'очарование', cc_fear: 'страх', cc_sleep: 'сон', cc_polymorph: 'превращение',
  ...
  dmg_reduction: 'снижение получаемого урона', cc_immune: 'иммунитет к контролю',
  cc_parry: 'парирование контроля', cc_spellshield: 'щит от умений',
  cc_unstoppable: 'неостановимость', cc_cleanse: 'снятие контроля',
  cc_tenacity: 'стойкость (короче контроль)', cc_immune_any: 'любое противодействие контролю',
...
for (const c of AB.champions) {
  ...
  for (const sp of c.spells) {
    ...
    /* ЗОНТИК. cc_hard = «полностью отнимает управление» — ставится, если сработал
       любой из точных видов. Так 52 правила «тег бьёт тег» и функции состава
       продолжают работать, не зная про разделение, а объяснение стало точным. */
    for (const t of UMBRELLA.cc_hard) {
      if (!tags.has(t)) continue;
      tags.add('cc_hard');
      if (!src.cc_hard) src.cc_hard = src[t];
      break;
    }
  }
