...
const RULES = [
  ...
  /* ИММУНИТЕТ К КОНТРОЛЮ. Раньше правило знало только «жёсткий контроль» и
     «обездвиживание» — и не видело ПОДБРОС. Из-за этой дыры Вуконг числился
     контрой Олафа, хотя всё наоборот: вся его угроза в ульте-подбросе, а Олаф
     в ульте к нему невосприимчив. Вес 5 — это не «полегче», это отмена. */
  ['cc_immune', 'cc_knockup', 5, 'иммунитет к контролю отменяет подброс — главный размен противника не проходит'],
  ['cc_immune', 'cc_stun', 4, 'оглушение по невосприимчивому к контролю не проходит'],
  ['cc_immune', 'cc_taunt', 4, 'насмешка не заставит бить не туда того, кто к контролю невосприимчив'],
  ['cc_immune', 'cc_charm', 4, 'очарование не уводит того, кто к контролю невосприимчив'],
  ['cc_immune', 'cc_fear', 4, 'страх не разворачивает того, кто к контролю невосприимчив'],
  ['cc_immune', 'cc_sleep', 4, 'усыпить невосприимчивого к контролю не выйдет'],
  ['cc_immune', 'cc_polymorph', 4, 'превращение по невосприимчивому к контролю не проходит'],
  /* ЩИТ ОТ УМЕНИЙ — съедает ЦЕЛОЕ умение вместе с его контролем. Это прямой
     ответ на однокнопочную угрозу: ульту-подавление, крюк, точечный контроль.
     Против длинного размена бесполезен — заряд один. */
  ['cc_spellshield', 'cc_suppress', 5, 'щит от умений съедает ульту-подавление целиком — размен не состоялся'],
  ['cc_spellshield', 'cc_pull', 5, 'крюк уходит в щит от умений — поймать не выйдет'],
  ['cc_spellshield', 'cc_point', 4, 'точечный контроль гасится щитом от умений'],
  ['cc_spellshield', 'cc_proj', 4, 'контроль-снаряд разбивается о щит от умений'],
  /* ПАРИРОВАНИЕ — блокирует и урон, и контроль, но окно короткое: работает
     против одного заряженного удара, а не против потока. */
  ['cc_parry', 'cc_hard', 4, 'парирование ловит заряженный контроль — вход противника пропадает'],
  ['cc_parry', 'cc_knockup', 4, 'подброс уходит в парирование'],
  /* НЕОСТАНОВИМОСТЬ — своё действие доводится до конца, но урон проходит. */
  ['cc_unstoppable', 'cc_knockback', 3, 'отбрасывание не срывает неостановимое движение'],
  ['cc_unstoppable', 'cc_pull', 3, 'притянуть неостановимого не выйдет'],
  ['cc_unstoppable', 'cc_slow', 3, 'замедление не мешает неостановимому доехать'],
  /* СНЯТИЕ КОНТРОЛЯ — работает ПОСЛЕ наложения: контроль прошёл, но снят. */
  ['cc_cleanse', 'cc_root', 4, 'обездвиживание снимается — цель уходит'],
  ['cc_cleanse', 'cc_stun', 4, 'оглушение снимается сразу после наложения'],
  ['cc_cleanse', 'cc_slow', 3, 'замедление снимается, догнать не выйдет'],
  /* СТОЙКОСТЬ — только СОКРАЩАЕТ длительность. Против подброса и подавления
     не работает вообще: это её главное ограничение, и путать его нельзя. */
  ['cc_tenacity', 'cc_stun', 2, 'стойкость режет длительность оглушения — размен короче'],
  ['cc_tenacity', 'cc_root', 2, 'стойкость сокращает обездвиживание'],
  ['cc_tenacity', 'cc_slow', 2, 'стойкость сокращает замедление'],
  /* НАСМЕШКА — не «ещё один стан». Она заставляет БИТЬ не туда, поэтому режет
     именно тех, кто живёт автоатаками, и почти безобидна для мага с умениями. */
  ['cc_taunt', 'auto_carry', 4, 'насмешка разворачивает автоатакера — его урон уходит не в ту цель'],
  ['cc_taunt', 'as_scaling', 3, 'вся скорость атаки бьёт не туда, пока держится насмешка'],
  /* ПОДАВЛЕНИЕ — самый жёсткий вид: цель не может вообще ничего.
     Обычная стойкость его не режет, спасают только иммунитет и рывок союзника. */
  ['cc_suppress', 'auto_carry', 5, 'подавление выключает керри целиком — ни ударить, ни уйти'],
  ['cc_suppress', 'immobile', 4, 'без рывка из подавления не выбраться — цель просто снимают'],
  /* СТРАХ и ОЧАРОВАНИЕ уводят цель С ПОЗИЦИИ — бьют по тем, кто держит строй. */
  ['cc_fear', 'frontline', 3, 'страх уводит танка с позиции — фронтлайн разваливается'],
  ['cc_charm', 'frontline', 3, 'очарование вытаскивает танка из строя, керри остаётся открытым'],
  ['cc_sleep', 'immobile', 3, 'сон по неподвижной цели — гарантированный размен'],
  ['cc_polymorph', 'auto_carry', 4, 'превращение отнимает и умения, и автоатаки — керри выключен'],
  ['cc_immune', 'cc_self_aoe', 4, 'контроль вокруг себя не берёт того, кто к контролю невосприимчив'],
  ['cc_immune', 'cc_hard', 4, 'иммунитет или чистка снимает главный контроль'],
  ['cc_immune', 'cc_root', 3, 'чистка снимает обездвиживание'],
  ['cc_immune', 'cc_pull', 3, 'притянуть можно, удержать — нет'],
  ['cc_immune', 'cc_point', 3, 'точечный контроль уходит впустую'],
