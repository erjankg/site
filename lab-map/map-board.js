...
(function() {
  ...
  // ───────────────────────────────────────────────────────────
  // Сетка строится ОДИН раз на список чемпов; поиск/роль только ПРЯЧУТ узлы.
  // Закон «НЕ ТАЩИТЬ ДЁРГАНЬЕ» п.5: никакого innerHTML на каждую букву поиска.
  const pickCards = new Map();   // имя чемпа → готовая кнопка
  let pickerBuiltFor = null;     // для какого массива чемпов построено
  let pickNoneEl = null;
  ...
  function renderPickerGrid(filter) {
    ...
    let shown = 0;
    for (const c of state.champions) {
      const el = pickCards.get(c.name);
      if (!el) continue;
      const ok = (role === 'all' || (c.is && c.is[role])) &&
                 (!q || c.name.toLowerCase().includes(q));
      el.hidden = !ok;
      el.classList.toggle('tb-pick-taken', taken.has(c.name));
      if (ok) shown++;
    }
  ...
  // Удаление объектов — только правым кликом (см. contextmenu-хендлер ниже).
  // Дабл-тап-удаление убрано по просьбе Эржана (мешало).
  ...
  // ───────────────────────────────────────────────────────────
  const MAP_KEY = 'labmap_strata_map_v1';
  const DEFAULT_MAP = '../tactics-board/assets/map-square.webp';
